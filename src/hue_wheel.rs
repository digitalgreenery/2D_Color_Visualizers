...
pub fn spawn(
    ...
    let (width, height) = (windows.single().width(),windows.single().height());
    ...
    for (stage, colors) in color_sets.into_iter().enumerate() {
        ...
        let arc_mesh = Mesh2dHandle(meshes.add(CircularSector::new(radius, arc_angle)));

        for (i, color) in colors.into_iter().enumerate() {
            let rotation_angle = -2. * i as f32 * arc_angle;
            commands.spawn(MaterialMesh2dBundle {
                mesh: arc_mesh.clone(),
                material: materials.add(color.to_linear_rgb().to_bevy_color()),
                transform: Transform::from_xyz(0.0, 0.0, 0.0 - stage as f32)
